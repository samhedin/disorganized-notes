...
## The note object
Below is a print of a basic note containing just a `Text` section.
...
;;Different sections have different keys and the format of the content key is different.
...
;;However, users can also add summations over rows/columns which are added as extra keys outside the content, 
;;since those are retained when cloning.

;;With that said, this is just a rule of thumb so check the cloning logic if necessary.
...
{:sections 
	...
	{e5c5c608-9b12-4794-bb28-9d5e80244dfc
		{section-position 0,  ;;Since sections are a map the position is saved this way.
		 ...
		 name Text, ;;Shown as the section title/header, rendered via edit.cljd
...
If you're working with dates, this can lead to unexpected behavior because if you send it a DateTime you'll get out a Timestamp, which is the corresponding Firestore type. Use `(.toDate x)` to get back your DateTime.



## Create a section
To create a section:
1. Add an entry to `model/default-section-map` which contains the basic data model.
2. Create the file `disorganized.ui.<section>` and add the logic.
3. Add an entry to `edit/add-section-button`.
